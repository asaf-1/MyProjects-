...
/**
 * LINK 1 — gov.il collectors/policies (Realty taxation)
 ...
 * - If same base filename exists but content is different => save as " (2).pdf", "(3).pdf", etc.
 * - Save page HTML + TEXT for every visited page
 */

// ===================== USER CONFIG =====================
const MAIN_URL =
  'https://www.gov.il/he/collectors/policies?officeId=c0d8ba69-e309-4fe5-801f-855971774a90&keywords=&TopicId=425269fd-fc92-4e85-a5ee-bc4cb368f74a';
const DOWNLOAD_DIR = 'C:/Users/asafn/Desktop/Tests-Playwright/tests/SCAPPER PROJECT/uri/Link 1';
const METADATA_CSV = path.join(DOWNLOAD_DIR, 'metadata_link1.csv');
...
test.setTimeout(60 * 60 * 1000); // up to 60 minutes
...
async function savePageContent(page: Page, meta: any, pageUrl: string) {
  ...
  if (htmlExists && txtExists) {
    console.log(`SKIP (content exists): ${base}.html / ${base}.txt`);
    return;
  }
...
async function downloadOnePdf(request: APIRequestContext, pdfUrl: string, baseFileName: string, meta: any, parentUrl: string) {
  if (SEEN_URLS.has(pdfUrl)) {
    ...
    return; // do not append CSV again
  }
...
// ===================== TEST =====================
test('LINK 1 — full crawl + collections + HTML/TXT capture', async ({ page, request }) => {
  await ensureDir(DOWNLOAD_DIR);
  await ensureDir(PAGE_HTML_DIR);
  await ensureDir(PAGE_TEXT_DIR);
