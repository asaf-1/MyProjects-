...
/**
 * LINK 3 — gov.il collectors/publications (Planning Administration)
 * - Full crawl with pagination
 * - Detect collection pages, follow /he/pages/ links to depth=1
 * - Download PDFs (persisted de-dup by URL via CSV)
 * - Save page HTML + TEXT for every visited page (גם כשאין PDF)
 * - Skip smartly empty/problem pages (e.g., page 16) and continue
 */

// ===================== USER CONFIG =====================
const MAIN_URL = 'https://www.gov.il/he/collectors/publications?officeId=2574c957-2109-4270-9586-62ca2d37857e';
const DOWNLOAD_DIR = 'C:/Users/Administrator/OneDrive/Desktop/Link 3';
const METADATA_CSV = path.join(DOWNLOAD_DIR, 'metadata_link3.csv');

// Crawl behavior
const MAX_PAGES = 999;                  // crawl until no next page
const MAX_DEPTH = 1;                    // follow /he/pages/ links from a collection up to depth 1
const MIN_PDFS_FOR_COLLECTION = 3;
const MIN_INTERNAL_LINKS_FOR_COLLECTION = 6;
const SKIP_PAGES = new Set<number>([16]); // ידוע שעמוד 16 בעייתי — מדלגים

// Network & pacing
const NAV_TIMEOUT = 45_000;
const REQUEST_TIMEOUT = 120_000;
const SLOW_PAUSE_MS = 350;
const MAX_EMPTY_STREAK = 5; // אם יש כמה עמודים ריקים ברצף — כנראה נגמר
...
async function savePageContent(page: Page, meta: any, pageUrl: string) {
  ...
  let htmlPath = path.join(PAGE_HTML_DIR, base + '.html');
  let txtPath  = path.join(PAGE_TEXT_DIR,  base + '.txt');

  // אם קיימים — לא נדרוס, נשמור עם (2)/(3) כדי לשמור היסטוריה רק במקרה נדיר של התנגשות שם
  if (fs.existsSync(htmlPath)) htmlPath = await uniquePath(htmlPath);
  if (fs.existsSync(txtPath))  txtPath  = await uniquePath(txtPath);
...
async function processDetailOrCollection(page: Page, request: APIRequestContext, href: string, metaSeed: any, depth: number, visited: Set<string>) {
  ...
  // Save page content (HTML + TEXT) — גם אם אין PDF
  try {
...
// Smart next-page logic: click next, or click number, or change query param; skip known bad pages
async function goToNextPageSmart(page: Page, currentPage: number): Promise<{moved:boolean, newPage:number}> {
  // skip known bad page(s)
  let target = currentPage + 1;
  while (SKIP_PAGES.has(target)) {
    console.log(`Skipping known problematic page ${target}…`);
    target += 1;
  }
  ...
  if (canClick && !disabled) {
    ...
    return { moved: true, newPage: currentPage + 1 };
  }

  // 2) try specific page button (target)
  const pageBtn = page.locator(`#GovilPaging button[title="עבור לעמוד מספר ${target}"]`).first();
  if (await pageBtn.isVisible().catch(() => false)) {
    console.log(`→ Jumping to page ${target}…`);
    await pageBtn.click();
    await page.waitForLoadState('networkidle');
    return { moved: true, newPage: target };
  }

  // 3) URL param fallback
  const paramNames = ['page', 'Page', 'pageIndex', 'PageIndex', 'pageindex', 'p'];
  for (const pn of paramNames) {
    try {
      const u = new URL(page.url());
      u.searchParams.set(pn, String(target));
      console.log(`→ Navigating via URL param ${pn}=${target}…`);
      await page.goto(u.toString(), { waitUntil: 'networkidle' });
      // minimal sanity check
      const hasResults = await page.locator('#Results .result-item').count().catch(() => 0);
      if (hasResults >= 0) {
        return { moved: true, newPage: target };
      }
    } catch { /* ignore and try next param */ }
  }

  // nothing worked — assume no more pages
  return { moved: false, newPage: currentPage };
}

// ===================== TEST =====================
test('LINK 3 — full crawl + collections + HTML/TXT capture', async ({ page, request }) => {
  await ensureDir(DOWNLOAD_DIR);
  await ensureDir(PAGE_HTML_DIR);
  await ensureDir(PAGE_TEXT_DIR);
  ...
  // עמוד ראשון
  const ok1 = await processResultsPage(page, request, current);
  emptyStreak = ok1 ? 0 : (emptyStreak + 1);

  // המשך עד שאין עוד עמודים
  while (current < MAX_PAGES) {
    const { moved, newPage } = await goToNextPageSmart(page, current);
    if (!moved) {
    ...
    const ok = await processResultsPage(page, request, current);
    if (!ok) {
      console.log(`No results found on page ${current}. Moving on…`);
      emptyStreak += 1;
      if (emptyStreak >= MAX_EMPTY_STREAK) {
        console.log('Too many empty pages in a row — probably end of list. Finishing.');
        break;
      }
      continue;
    }
